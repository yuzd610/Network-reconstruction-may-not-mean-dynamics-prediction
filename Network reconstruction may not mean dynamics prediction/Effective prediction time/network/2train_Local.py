...
# train for different data volumes
for i in tqdm(range(15, 50, 1)):
    p.append(125 * i)
    q = np.zeros(r)
...
mean_dw = np.mean(dw, axis=1)
std_d_n = np.std(dw, axis=1)




np.save('mean_dw.npy', mean_dw)
np.save('p.npy', p)
np.save('std_d_n.npy',std_d_n )
