...
#iterative trajectory simulation
for i in tqdm(range(n)):
    ...
    J_1 = np.random.normal(loc=0, scale=np.sqrt((g ** 2) / N1), size=(N1, N1))
    J_11 = np.random.normal(loc=0, scale=np.sqrt((g ** 2) / N1), size=(N1, N1))
...
mean_d_n = np.mean(d_n, axis=0)
std_d_n = np.std(d_n, axis=0)
