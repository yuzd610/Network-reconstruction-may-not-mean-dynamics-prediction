...
class DynamicModel(nn.Module):
    ...
    def forward(self, trajectory):
        ...
        for p in range(reconstruct_trajectory.shape[2] - 1):
            ...
            # Calculate the slope of RK4
            k1 = self._dx_dt(current_points)
            k2 = self._dx_dt(current_points + 0.5 * delta_t * k1)
            k3 = self._dx_dt(current_points + 0.5 * delta_t * k2)
            k4 = self._dx_dt(current_points + delta_t * k3)

            # RK4 update formula
            next_points = current_points + (delta_t / 6) * (k1 + 2 * k2 + 2 * k3 + k4)

            reconstruct_trajectory[:, :, p + 1] = next_points

        return reconstruct_trajectory

    def _dx_dt(self, points):
        """Calculate dx/dt = -points + W * tanh(points)"""
        return -points + torch.matmul(self.weight, torch.tanh(points.t())).t()
